...
import {
  ...
  PlannedGymSession,
  PlannedGymSessionSchema,
  WeekActivities,
  WeekActivitiesSchema,
} from "./schema.js";
...
export async function deleteGymSession(date: string): Promise<void> {
  try {
    await fs.unlink(paths.gymSession(date));
  } catch {
...
export async function readPlanForWeek(weekStart: string): Promise<Plan | null> {
  // Try the week-specific file first, fall back to current.json if week matches.
  const weekFile = paths.planWeek(weekStart);
  let raw = await readJson<unknown>(weekFile);
  if (!raw) {
    const cur = await readJson<unknown>(paths.planJson());
    if (cur && (cur as any).week_start === weekStart) raw = cur;
  }
  return raw ? PlanSchema.parse(raw) : null;
}

export async function listPlanWeeks(): Promise<string[]> {
  const dir = paths.data() + '/plan';
  const files = (await listFiles(dir)).filter((f) => /^\d{4}-\d{2}-\d{2}\.json$/.test(f));
  return files.map((f) => f.replace('.json', '')).sort();
}

export async function writePlan(plan: Plan): Promise<void> {
  const parsed = PlanSchema.parse(plan);
  // Write both the week-specific file and current.json so existing tooling keeps working.
  await Promise.all([
    writeJsonAtomic(paths.planJson(), parsed),
    writeJsonAtomic(paths.planWeek(parsed.week_start), parsed),
  ]);
}
...
/* ----------------------- planned gym sessions ----------------------- */

export async function readPlannedSession(date: string): Promise<PlannedGymSession | null> {
  const raw = await readJson<unknown>(paths.plannedSession(date));
  if (!raw) return null;
  try { return PlannedGymSessionSchema.parse(raw); } catch { return null; }
}

export async function writePlannedSession(session: PlannedGymSession): Promise<void> {
  await ensureDir(paths.plannedSessionsDir());
  await writeJsonAtomic(paths.plannedSession(session.date), PlannedGymSessionSchema.parse(session));
}

export async function deletePlannedSession(date: string): Promise<void> {
  try { await fs.unlink(paths.plannedSession(date)); } catch {}
}

export async function listPlannedSessionsForWeek(weekStart: string): Promise<PlannedGymSession[]> {
  const dir = paths.plannedSessionsDir();
  const files = await listFiles(dir);
  const d = new Date(weekStart + 'T12:00:00Z');
  const weekEnd = new Date(d);
  weekEnd.setUTCDate(weekEnd.getUTCDate() + 7);
  const endStr = weekEnd.toISOString().slice(0, 10);
  const sessions: PlannedGymSession[] = [];
  for (const f of files) {
    if (!f.endsWith('.json')) continue;
    const date = f.replace('.json', '');
    if (date >= weekStart && date < endStr) {
      const raw = await readJson<unknown>(`${dir}/${f}`);
      if (raw) {
        try { sessions.push(PlannedGymSessionSchema.parse(raw)); } catch {}
      }
    }
  }
  return sessions.sort((a, b) => a.date.localeCompare(b.date));
...
export async function readWeekActivities(weekStart: string): Promise<WeekActivities | null> {
  const raw = await readJson<unknown>(paths.weekActivities(weekStart));
  if (!raw) return null;
  try { return WeekActivitiesSchema.parse(raw); } catch { return null; }
}

export async function writeWeekActivities(activities: WeekActivities): Promise<void> {
  await ensureDir(paths.weekActivitiesDir());
  await writeJsonAtomic(paths.weekActivities(activities.week_start), WeekActivitiesSchema.parse(activities));
}
