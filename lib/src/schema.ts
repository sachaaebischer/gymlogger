...
// ── Planned gym sessions (generated by web app, lives in data/plan/sessions/) ──
...
export const PlannedExerciseSchema = z.object({
  ...
  target_weight_kg: z.number().nullable().default(null),
  target_weight_label: z.string().default(""),
...
export const PlannedGymSessionSchema = z.object({
  date: dateStr,
  session_type: z.enum(["gym_upper", "gym_lower"]),
  generated_at: z.string(),
  week_notes: z.string().default(""),
  exercises: z.array(PlannedExerciseSchema).default([]),
});
export type PlannedGymSession = z.infer<typeof PlannedGymSessionSchema>;

// ── Week activities (user input: what else is happening that week) ──

export const DayActivitySchema = z.object({
  type: z.string(),
  label: z.string(),
  intensity: z.enum(["high", "medium", "low"]).default("medium"),
  notes: z.string().default(""),
});
export type DayActivity = z.infer<typeof DayActivitySchema>;

export const WeekActivitiesSchema = z.object({
  week_start: dateStr,
  days: z.record(z.array(DayActivitySchema)).default({}),
});
export type WeekActivities = z.infer<typeof WeekActivitiesSchema>;
